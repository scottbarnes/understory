...
ENV DJANGO_SETTINGS_MODULE=understory.settings.production DJANGO_DEBUG=off

# Tell uWSGI where to find your wsgi file:
ENV UWSGI_WSGI_FILE=understory/wsgi.py
...
ENV UWSGI_STATIC_MAP="/media/=/code/understory/media/"
...
RUN mkdir -p /code/understory/media/images && chown -R 1000:2000 /code/understory/media

# mark the destination for images as a volume
VOLUME ["/code/understory/media/images/"]
